...
api.get(
  ...
  asyncHandler(async (_req, res) => {
    ...
    res.json({
      ...
      plans: plans
        .filter((p) =>
          cycles.some((c) => Number(c === 'MONTHLY' ? p.monthly_price : p.yearly_price) > 0),
        )
        .map((p) => ({
          id: p.id,
          name: p.name,
...
api.post(
  ...
  validate(
    z.object({
      ...
      replacement: z
        .object({ amount: paymentAmount, method: paymentMethod, note: z.string().nullable().optional() })
        .optional(),
    }),
  ),
  asyncHandler(payments.amend),
);
