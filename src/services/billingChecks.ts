...
export function matchAccount(
  receiptValue: string | null | undefined,
  configured: string | null | undefined,
  kind: 'phone' | 'account',
): boolean | null {
  if (!receiptValue || !configured) return null;

  const a = canonical(receiptValue, kind);
  const b = canonical(configured, kind);
  if (!a || !b) return null;

  if (informativeDigits(a, kind) < 4 || informativeDigits(b, kind) < 4) return null; // too masked to judge
  ...
  if (n >= 4) return runA.slice(-n) === runB.slice(-n);
 ...
 */
function informativeDigits(value: string, kind: 'phone' | 'account'): number {
...
function canonical(value: string, kind: 'phone' | 'account'): string | null {
  const cleaned = value
    .trim()
    .replace(/[xX]/g, '*')
    .replace(/[^0-9*]/g, '');
...
function canonicalPhone(value: string): string {
  ...
  if (value.startsWith('0') && value.length >= 9) return `251${value.slice(1)}`;
  if (value.length === 9 && /^[79*]/.test(value)) return `251${value}`;
  return value;
}

function trailingVisibleRun(value: string): string {
...
export function priceFor(plan: BillingPlanRow, cycle: BillingCycle): number {
 ...
 */
export function resolveCycle(paid: number, plan: BillingPlanRow, tolerance = 0.01): BillingCycle | null {
  const yearly = priceFor(plan, 'YEARLY');
  const monthly = priceFor(plan, 'MONTHLY');
  if (yearly > 0 && paid + tolerance >= yearly) return 'YEARLY';
  if (monthly > 0 && paid + tolerance >= monthly) return 'MONTHLY';
  return null;
}
...
export interface CheckInput {
  ...
  /** Why the receipt could not be read, when `receipt` is null. */
...
export interface CheckOutcome {
  ...
  warnings: string[];
  passed: boolean;
  /** Headline reason — the FIRST failing rule. */
  failureReason: string | null;
  /** What the money actually bought; null when the amount rule did not pass. */
  grantedCycle: BillingCycle | null;
}
 ...
 */
export function runChecks(input: CheckInput): CheckOutcome {
  ...
  if (!receipt.reason) {
    checks.push({
      ...
      actual: null,
      message:
        `That receipt has no note on it, so we cannot tell it is your payment. ` +
    ...
    checks.push({
      ...
      actual: receipt.reason,
      message:
        `The reason says "${receipt.reason}", which does not contain your code ${reasonCode}. ` +
        `Send the payment again and type ${reasonCode} in the "reason" box before confirming.`,
    });
  }

  // --- amount (guarantee 3) ----------------------------------------------
  let grantedCycle: BillingCycle | null = null;
  if (!plan) {
    checks.push({
      key: 'amount',
      label: 'Amount paid',
    ...
    if (grantedCycle) {
      ...
      checks.push({
        ...
        message:
          grantedCycle === input.selectedCycle
      ...
      if (grantedCycle !== input.selectedCycle) {
        warnings.push(
          `Selected ${input.selectedCycle.toLowerCase()} but paid enough for ${grantedCycle.toLowerCase()}.`,
        );
      }
    } else {
      checks.push({
        ...
        actual: money(receipt.amount, currency),
        message:
          `That is less than the ${money(cheapest, settings.currency)} needed for the cheapest ${plan.name} ` +
  ...
  const kind: 'phone' | 'account' = input.provider === 'CBE' ? 'account' : 'phone';
  ...
  if (!receipt.date) {
    ...
    if (ageDays < -1) {
      ...
      checks.push({
        ...
        actual: shown,
        message:
          `That receipt is ${Math.floor(ageDays)} days old. Only receipts from the last ${maxAgeDays} ` +
