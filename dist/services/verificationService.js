 ...
 * second time.
 */
const MISCONFIGURED = 'Receipt verification is not set up correctly on the server, so this receipt could not be checked. ' +
...
function errorCode(err) {
    const e = err;
    return e?.cause?.code ?? e?.code ?? err?.name ?? 'UNKNOWN';
}
/**
    ...
    for (let attempt = 0; attempt <= env_1.env.verification.retries; attempt += 1) {
        ...
        try {
            ...
            if (UNREACHABLE.has(code)) {
                console.error(`[verify] cannot reach ${url} (${code}). VERIFY_API_URL is wrong or the host is down.`);
            ...
            console.warn(`[verify] transport error on ${url} (attempt ${attempt + 1}) [${code}]:`, err.message);
        ...
        if (!res.ok && !isJson) {
            console.error(`[verify] non-JSON ${res.status} from ${url} ` +
