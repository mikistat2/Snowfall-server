...
async function list(gymId, filter = {}) {
    ...
    const base = (0, knex_1.db)('audit_logs as a').where('a.gym_id', gymId);
    if (filter.entity)
        base.andWhere('a.entity', filter.entity);
    if (filter.action)
        base.andWhereILike('a.action', `%${filter.action}%`);
    const [countRow] = await base.clone().count('a.id as count');
